...
>
> 문서 목적: Musixmatch, Deezer, Bugs, Genie 네 공급자를 안전하게 단계 도입하기 위한 구현 청사진과 검토 체크리스트

## 1. 결론과 기본 결정

기존 `LyricsRepository`의 Spotify 메타데이터 조회, ivLyrics `sync-data`/OpenDB, LRCLIB 흐름은 유지한다. 네 신규 공급자와 기존 LRCLIB는 공통 `LyricsProvider` 경계 뒤의 어댑터로 다루고, `LyricsProviderOrchestrator`가 조회 순서·동시 실행·취소·오류 격리를 담당한다. `LyricsRepository`는 유효 정책 결정, 허용된 캐시 선조회, 필요한 경우의 메타데이터 및 `sync-data` 단일 조회, 오케스트레이터 호출, 노래방 보강을 조정하는 얇은 계층으로 점진적으로 바꾼다.
...
6. 실험 모드도 `sync-data.lrclibId`가 있으면 LRCLIB 직접 조회를 가장 먼저 보존한다. 그 뒤 Musixmatch를 우선 시도하고, Deezer·Bugs·Genie·LRCLIB 검색 어댑터를 제한된 동시성의 fallback으로 명시적으로 포함한다.
...
10. 네 신규 공급자 모두 비공식·사설·스크래핑 방식의 엔드포인트를 사용하므로 서비스 약관, 가사 권리, App Store 심사는 출시 차단 조건이다.
...
- Musixmatch, Deezer, Bugs, Genie의 검색·후보 매칭·가사 조회·정규화
...
Spotify Web API는 가사를 제공하지 않는다. `spclient color-lyrics`는 별도의 비공식 다섯 번째 공급자 후보일 뿐이며 이번 네 공급자 통합 범위에는 넣지 않는다.
...
7. cache miss이고 effective mode가 multiProvider면
   ...
   c. Deezer/Bugs/Genie/LRCLIB 검색을 동시성 2의 대기열로 수집
...
참조 자동 모드는 Musixmatch 동기화 가사를 먼저 확정하고, 그렇지 않으면 설정된 Deezer와 Bugs·Genie를 함께 기다린 뒤 공급자 순서상 첫 동기화 결과, 없으면 첫 일반 결과를 고른다. iOS에서는 그대로 `join all`만 하지 않는다.

**고정 기본 정책:** `.multiProvider`의 일반 공급자 순서는 `[musixmatch, deezer, bugs, genie, lrclib]`이다. Deezer가 구성·활성화되지 않았으면 건너뛴다. `sync-data.lrclibId` 직접 조회는 이 순위보다 앞선 호환성 preflight이며, LRCLIB 검색은 fallback 순위의 마지막에 명시적으로 남는다. 향후 사용자 설정이나 실험이 순서를 바꿀 수는 있지만, 정규화된 유효 순서가 없으면 반드시 이 기본값으로 돌아간다.
...
- `sync-data.lrclibId` 직접 preflight, Musixmatch 우선, Deezer/Bugs/Genie/LRCLIB 검색 fallback 결합
...
- LRCLIB 검색이 Deezer/Bugs/Genie와 함께 명시적 fallback으로 참여
...
- [ ] 네 비공식 공급자를 앱에 직접 포함해도 되는지 법무·서비스 약관·App Store 검토 완료
